...
<body>
	<c:if test="${sessionScope.user != null}">
		<form
			action="/PBL3/user-job?locationS=<%=request.getParameter("locationS")%>&typeS=<%=request.getParameter("typeS")%>&categoryS=<%=request.getParameter("categoryS")%>"
			method="get">
	</c:if>
	<c:if test="${sessionScope.user == null}">
		<form
			action="/PBL3/job-listing?locationS=<%=request.getParameter("locationS")%>&typeS=<%=request.getParameter("typeS")%>&categoryS=<%=request.getParameter("categoryS")%>"
			method="get">
	</c:if>
								...
								<div class="single-job-items mb-30">
									<div class="job-items">
										<div class="company-img">
											<c:if test="${sessionScope.user != null}">
												<a
													href="/PBL3/user-recruiter-resume?idRecruiter=${job.idRecruiter}"><img
													src="${pageContext.request.contextPath}<c:out value="${job.recruiterDetail.img}" />"
													alt="" style="width: 100px; height: 100px;"></a>
											</c:if>
											<c:if test="${sessionScope.user == null}">
												<a
													href="/PBL3/guest-recruiter-resume?idRecruiter=${job.idRecruiter}"><img
													src="${pageContext.request.contextPath}<c:out value="${job.recruiterDetail.img}" />"
													alt="" style="width: 100px; height: 100px;"></a>
											</c:if>
										</div>
									...
									<div class="items-link items-link2 f-right">
										...
										<c:if test="${sessionScope.user == null}">
											<a href="/PBL3/job-details?id=${job.id}" />
											<c:out value="${job.type.name}" />
											</a>
										</c:if>
								...
								<c:if test="${page > 1}">
									<c:if test="${sessionScope.user != null}">
										<li class="page-item"><a class="page-link"
											href="/PBL3/user-job?locationS=${param.locationS}&typeS=${param.typeS}&categoryS=${param.categoryS}&page=${page - 1}">Previous</a>
										</li>
									</c:if>
									<c:if test="${sessionScope.user == null}">
										<li class="page-item"><a class="page-link"
											href="/PBL3/job-listing?locationS=${param.locationS}&typeS=${param.typeS}&categoryS=${param.categoryS}&page=${page - 1}">Previous</a>
										</li>
									</c:if>
								</c:if>
										...
										<c:when test="${temp == page}">
											<c:if test="${sessionScope.user != null}">
												<li class="page-item active"><a class="page-link"
													href="/PBL3/user-job?locationS=${param.locationS}&typeS=${param.typeS}&categoryS=${param.categoryS}&page=${temp}">${temp}</a>
												</li>
											</c:if>
											<c:if test="${sessionScope.user == null}">
												<li class="page-item active"><a class="page-link"
													href="/PBL3/job-listing?locationS=${param.locationS}&typeS=${param.typeS}&categoryS=${param.categoryS}&page=${temp}">${temp}</a>
												</li>
											</c:if>
										</c:when>
										<c:otherwise>
